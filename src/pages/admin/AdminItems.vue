    ...
    <div class="admin-content">
      ...
      <div class="filters">
        ...
        <div class="filter-buttons">
          ...
          <button
            :class="{ active: filterType === 'suggested' }"
      ...
      <!-- Table -->
      <div class="table-card" v-if="!loading && filteredItems.length">
        <table class="item-table">
          <thead>
            <tr>
              ...
              <th>Status</th>
              <th>Action</th>
              <th v-if="filterType === 'suggested'">Suggested Owner</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="item in filteredItems" :key="item.id">
              ...
              <td v-if="filterType === 'suggested'">
                {{ item.suggested_owner?.firstname || "Not found" }}
                {{ item.suggested_owner?.lastname || "" }}
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <!-- Loading / No items -->
      <p v-if="loading" class="loading-text">Loading items...</p>
      <p v-else-if="!filteredItems.length" class="loading-text">
        No items found.
      </p>
    </div>
  </div>
    ...
    async fetchItems() {
      ...
      try {
        ...
        if (this.filterType === "suggested") {
          url = "/admin/items/suggested-owner?per_page=50";
        }
        const response = await axios.get(url);

        // Handle paginated response for suggested owners
        if (this.filterType === "suggested") {
    ...
    filteredItems() {
      ...
      return this.items.filter((item) => {
        ...
        const suggestedFirstname =
          item.suggested_owner?.firstname?.toLowerCase() || "";
        const suggestedLastname =
          item.suggested_owner?.lastname?.toLowerCase() || "";

        // Filter by type
        if (this.filterType === "lost" && type !== "lost") return false;
        if (this.filterType === "found" && type !== "found") return false;
        if (this.filterType === "suggested" && !item.suggested_owner)
          return false;
        ...
        return (
          ...
          suggestedFirstname.includes(query) ||
          suggestedLastname.includes(query)
        );
      });
    },
  },
